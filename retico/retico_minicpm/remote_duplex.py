...
try:
    ...
    # duplex_client.py lives one directory up (retico/); make it importable
    # even when this package is imported from a different working directory.
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from duplex_client import FullDuplexClient

# LAN duplex server (server.py --host 0.0.0.0 --port 8998 on the vLLM box);
# override per-instance or via DUPLEX_SERVER_URL in test.py.
DEFAULT_SERVER_URL = "ws://192.168.0.179:8998"
