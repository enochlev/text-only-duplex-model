...
|---|---|---|---|---|
| 2026-07-13 | **Run 8 result** (`run8_stripFALSE_baseRM2_180`, winning config) | — | ✅ VALIDATED | 180 steps, strip=False + baseline RM2. best avg_reward **+1.36**; RM2 interruption **−2.37→−0.10**, RM4 timely **+0.99**, RM1 −0.06, RM6 0.0; non_idle settled ~0.40. Replay-eval on frozen 5-rec eval_set (both monotonic ASR): **stale/spoke base 0.617 → trained 0.315** (cut-offs ~halved) and trained speaks MORE (54 vs 47 blocks) → NOT over-silent. `best/` = the model to serve. |
| 2026-07-13 | Run 9 (`run9_seed2_stripFALSE_baseRM2_180`) | — | ✅ REPRODUCES | 2nd independent seed of the Run 8 config. best avg_reward **+1.35** (vs Run 8 +1.36), RM2 −0.15, RM4 +0.80 — matches. Replay-eval: **stale/spoke 0.362** (vs Run 8 0.315, base 0.617); stale=17 blocks identical to seed 1. → the turn-taking gain is seed-robust, not noise. 3-way eval numbers are the paper result. |
| 2026-07-13 | Monotonic ASR commit (`full_duplex.py` `_ASR_MONOTONIC_COMMIT`, env-gated, default off) | — | added | Time-cursor freeze: any block ending before `window_end − _ASR_RIGHT_CONTEXT_S`(2s) is frozen & never rewritten, replacing the block-granular mutable-window + Fix A/C churn heuristics. Word timestamps + flush semantics preserved. Offline (`asr_compare.py`): legacy rolling ~28 retro-edits/10s → 0. Live A/B: stale 31→25, **flush count unchanged** (prod Fix-B already gated most churn) → modest win + big code simplification, NOT a cut-off-rate drop. Enable with `ASR_MONOTONIC_COMMIT=1`. |
| 2026-07-13 | `run_demo.py` + `run_demo_ui.html` survey front-end | gradio `survey_demo.py` | added (non-gradio) | Clean single-page survey: consent gate (required checkbox), blinded A/B (one Start/Stop btn, conn-status pill, You/Bot volume meters, live transcript), Likert + comparison, JSONL responses, FRP-tunnel served (no gradio UI). Flags `--model_a_url --model_b_url --enable_free_chat --share`. Also fixed `server.py --port` (was hardcoded 8998). See `SURVEY.md`. |
| 2026-07-12 (eve) | Punctuation strip (`full_duplex.py` `_STRIP_USER_PUNCTUATION`) | stripped (True) | **KEPT (False)** | REVERTS the AM strip. Live testing: base MiniCPM-duplex uses terminal punctuation as its turn-end "safe to talk" cue — with strip ON even the un-RL'd base went mute on 3/4 casual turns (`-q fp8` + greedy made it worse; serve bf16). Parakeet supplies punctuation timely in prod. ALL strip=True RL runs (2/4/5/6) are now invalid (train/serve mismatch) and scratched. |
| 2026-07-12 (eve) | RM2 run≥2 penalties (`rewards.py`) | steep −1.5/−2.25/−3.0 | baseline −1.0/−1.5/−2.0 | Reverts the AM steepen. Steep over-silenced; strip=False restores the turn cue so keep RM2 mild (base only slightly over-interrupts). Fresh strip=False run `run8_stripFALSE_baseRM2_180` from scratch (ports 8700). Eval via `replay_client.py` on the frozen 5-recording eval_set (base baseline: stale/spoke≈0.55). |
| 2026-07-12 | RM2 run≥2 penalties (`rewards.py` `block_interruption_penalty`) | run2=−1.0, run3=−1.5, run≥4=−2.0 | run2=−1.5, run3=−2.25, run≥4=−3.0 | Run 4 (winning config, 180 steps) still left persistent barge-in (late RM2≈−0.47); steepen the escalation so repeat-interrupts cost more. run1 (single mistimed word) kept cheap at −0.75. Run 5 A/B-tests this vs the Run 4 baseline. Commit 75e4234 |
| 2026-07-12 | Forced-speech epsilon (`rl_trainer.py`) | 0.20 (tested 0.10) | 0.20 (kept) | Run 3 A/B: forced-speech 0.10 gave no measurable benefit vs 0.20 at convergence (non_idle/RM trajectories overlapped); reverted. Commits 53a8729→e0f33cc |
| 2026-07-12 | Run archive disposition (`~/scratch/duplex_runs`) | run1/run2/run3 kept | run1+run3 removed; run4 = new best-so-far | Run 4 (180-step replication of Run 2's rebalanced config) beat Run 2 on every summary metric (all-steps −0.52 vs −0.60, late −0.17 vs −0.22, best avg_reward +0.144) → new best. Deleted run1_rebalance_90step (superseded by run2) and run3_forcespeak010_120step (no-benefit A/B). summary.txt added to each kept run |
