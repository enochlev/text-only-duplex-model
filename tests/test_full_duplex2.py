"""
Tests for DuplexAudioAgent (full_duplex2.py).
...
def test_format_timeblocks_forced_words():
    """Forced words (pending[:N]) appended after <AI> in current window."""
    agent = make_agent()  # N=5
    agent._pending_words = ["hello", "world", "how", "are", "you", "extra"]
    result = agent._format_timeblocks()
    # forced = pending[:5] = ["hello", "world", "how", "are", "you"]
    assert result.endswith("<user><idle><AI>hello world how are you")
...
def test_mic_rolling_buffer_capped():
    ...
    from full_duplex2 import MAX_MIC_BLOCKS
...
def test_receive_text_message_appends():
    agent = make_agent()
    agent.receive_text_message("first")
    agent._committed_words = []  # reset manually to simulate same utterance
    agent.context_version -= 1  # undo bump for this test
    # Second message appends
    agent._ensure_current_block(1000.0)
    agent._current_block.user_text += " second"
    assert agent._current_block.user_text == "first second"
